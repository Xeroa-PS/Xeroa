...
#include "game/packets/PlayerTokenReq.h"
#include "game/packets/PlayerLoginReq.h"
#include "game/packets/PingReq.h"
		...
		if (!this->packet_queue.empty())
		{
			...
			switch (packet.m_PacketId)
			{
				...
				case PacketIds::PlayerLoginReq:
					HandlePlayerLoginReq(this, packet);
					break;
				default:
					break;
			}
...
{
	auto encrypted = Xor(data.GetDataOwnership(), use_dispatch_key ? DispatchKey : this->mt_key);
